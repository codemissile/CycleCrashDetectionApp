    ...
    public partial class MainPage : ContentPage
    {
        ...
        private void CheckForPotentialCrash()
        {
            ...
            if (sensorDataList.Count >= 2)
            {
                ...
                // Debug logs
                //Debug.WriteLine($"Current AccelMag: {currentData.AccelMag}");
                //Debug.WriteLine($"Current GyroMag: {currentData.GyroMag}");

                // Check if the magnitudes exceed the threshold
                if (currentData.AccelMag > 1.0 && currentData.GyroMag > 1.0)
                {
        ...
        {
            if (string.IsNullOrWhiteSpace(RecipientBox.Text) || string.IsNullOrWhiteSpace(MessageBox.Text))
            {
                await DisplayAlert("Error", "Please enter both a recipient and a message.", "OK");
                return;
            }

            var message = new SmsMessage(MessageBox.Text.Trim(), new[] { RecipientBox.Text.Trim() });
